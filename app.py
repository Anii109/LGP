...
# =================== TMDB API KEY ===================
API_KEY = "e6ba2f139288bd08eac320a78add299a"
...
def recommend(movie):
    ...
    for i in movie_list:
        movie_id = movies_df.iloc[i[0]].movie_id
        details = fetch_movie_details(movie_id)
        details["trailer_url"] = fetch_movie_trailer(movie_id)
        recommended.append(details)
    return recommended

# =================== Save Rating to CSV ===================
def save_rating(selected_movie, rated_movies):
    file_name = "ratings.csv"
    columns = ["selected_movie", "recommended_movie", "user_rating", "timestamp"]
    rows = []

    for title, rating in rated_movies:
        rows.append([selected_movie, title, rating, datetime.now().isoformat()])
...
if st.button("Recommend"):
    ...
    for movie in recommendations:
        ...
        with cols[1]:
            ...
            # ========== Rating Slider ==========
            user_rating = st.slider(f"Your Rating for '{movie['title']}'", 1, 5, 3, key=movie['id'])
            rating_store.append((movie["title"], user_rating))
